class Game {
  ...
  start() {
    ...
    this.addSprites(lifes, 3 , lifeImg, 0.42);
  ...
  handleElements() {
    ...
    this.menuButton.position(width / 2 + 230, 100);

    this.muteButton.class("muteButton");
    this.muteButton.position()
  ...
  play() {
    ...
    if (allPlayers !== undefined) {
      image(track, 7, -height * 3, width, height * 10);
      ...
      for (var plr in allPlayers) {
        ...
        if (index === player.index) {
          ...
          image(markerImg,x-40,y-120,100,100);
      ...
      // Finshing Line
      const finshLine = height * 4 - 5;

      if (player.positionY > 2550) {
        gameState = 2;
        player.rank += 1;
  ...
  handleLife(index) {
    ...
    cars[index - 1].overlap(lifes, function(collector, collected) {
      player.life=185;
